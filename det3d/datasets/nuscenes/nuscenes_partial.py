...
class NuScenesPartialDataset(PointCloudDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(NuScenesPartialDataset, self).__init__(
        ...
        self._num_point_features = NuScenesPartialDataset.NumPointFeatures
    ...
    def load_infos(self, info_path):
        ...
        if not self._active_buffer.endswith(".json"):
            print("load all data to train")
        elif self._active_flag == "start":
            print(
                f"begin sample, random sample 0.1 ratio "
                f"of dataset to start active learning "
                f"save the 'sample_ids' in {self._active_buffer}")
            self.ratio = 0.1
            num_sample = int(len(_nusc_infos_all) * self.ratio)
            sample_ids = random.sample(range(num_sample), num_sample)
            buffer =  {"partial_01": sample_ids}
            dump(buffer, self._active_buffer, indent=4)
            _nusc_infos_all_partial = []
            for i in sample_ids:
                _nusc_infos_all_partial.append(_nusc_infos_all[i])
            _nusc_infos_all = _nusc_infos_all_partial
        else:
            print(
                f"load active learning buffer: {self._active_buffer}"
                f"and the flag is {self._active_flag}")
            active_buffer = load(self._active_buffer)
            sample_ids = active_buffer[self._active_flag]
            _nusc_infos_all_partial = []
            for i in sample_ids:
                _nusc_infos_all_partial.append(_nusc_infos_all[i])
            _nusc_infos_all = _nusc_infos_all_partial
