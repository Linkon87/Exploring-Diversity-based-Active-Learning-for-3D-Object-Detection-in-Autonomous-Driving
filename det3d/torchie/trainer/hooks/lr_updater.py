...
class LrUpdaterHook(Hook):
    ...
    def before_run(self, trainer):
        ...
        try:
            for group in trainer.estimator_optimizer.param_groups:
                group.setdefault("initial_lr", group["lr"])
            self.base_lr = [group["initial_lr"] for group in trainer.estimator_optimizer.param_groups]
        except:
            pass
    ...
    def _set_estimator_lr(self, trainer, lr_groups):
        for param_group, lr in zip(trainer.estimator_optimizer.param_groups, lr_groups):
            param_group["lr"] = lr

    def before_estimator_train_epoch(self, trainer):
        if not self.by_epoch:
            return
        self.regular_lr = self.get_regular_lr(trainer)
        self._set_estimator_lr(trainer, self.regular_lr)

    def before_estimator_train_iter(self, trainer):
        cur_iter = trainer.iter
        if not self.by_epoch:
            self.regular_lr = self.get_regular_lr(trainer)
            if self.warmup is None or cur_iter >= self.warmup_iters:
                ...
                warmup_lr = self.get_warmup_lr(cur_iter)
                self._set_estimator_lr(trainer, warmup_lr)
        elif self.by_epoch:
            if self.warmup is None or cur_iter > self.warmup_iters:
                return
            elif cur_iter == self.warmup_iters:
