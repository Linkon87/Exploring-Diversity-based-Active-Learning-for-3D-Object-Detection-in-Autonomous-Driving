...
class ActiveTrainer(object):
    ...
    def __init__(
        ...
        lr_scheduler=None,
        estimator=None,
        estimator_optimizer=None,
        estimator_lr_scheduler=None,
        work_dir=None,
        ...
        **kwargs,
        ...
        self.estimator_optimizer = estimator_optimizer
        self.estimator_lr_scheduler = estimator_lr_scheduler
    ...
    def current_estimator_lr(self):
        if self.estimator_optimizer is None:
            raise RuntimeError("lr is not applicable because estimator_optimizer does not exist.")
        return [group["lr"] for group in self.estimator_optimizer.param_groups]
    ...
    def estimator_load_checkpoint(self, filename, map_location="cpu", strict=False):
        self.logger.info("load estimator checkpoint from %s", filename)
        return load_checkpoint(self.estimator, filename, map_location, strict, self.logger)
    ...
    def estimator_save_checkpoint(
        self, out_dir, filename_tmpl="estimator_epoch_{}.pth", save_optimizer=True, meta=None
    ):
        if meta is None:
            meta = dict(epoch=self.epoch + 1, iter=self.iter)
        else:
        ...
        filepath = osp.join(out_dir, filename)
        linkpath = osp.join(out_dir, "latest.pth")
        estimator_optimizer = self.estimator_optimizer if save_optimizer else None
        save_checkpoint(self.estimator, filepath, optimizer=estimator_optimizer, meta=meta)
        # Use relative symlink
        torchie.symlink(filename, linkpath)
    ...
    def estimator_batch_processor_inline(self, model, estimator, data, train_mode, **kwargs):
        ...
        if train_mode:
            with torch.no_grad():
                preds, middle = model(example, return_loss=False, estimate=True)
                example["preds"] = preds
                example["middle"] = middle
            losses = estimator(example, return_loss=True)
            self.call_hook("after_forward")
            loss, log_vars = parse_second_losses(losses)
            del losses
            ...
            preds = model(example, return_loss=False)
            example["preds"] = preds
            return estimator(example, return_loss=False)
    ...
    def estimator_train(self, data_loader, epoch, **kwargs):
        ...
        self.length = len(data_loader)
        self._max_iters = self._max_epochs * self.length
        self.call_hook("before_estimator_train_epoch")

        epoch_bias = kwargs["epoch_bias"]
        base_step = (epoch - epoch_bias) * self.length

        # prefetcher = Prefetcher(data_loader)
        # for data_batch in BackgroundGenerator(data_loader, max_prefetch=3):
        for i, data_batch in enumerate(data_loader):
            global_step = base_step + i
            if self.estimator_lr_scheduler is not None:
                self.estimator_lr_scheduler.step(global_step)

            self._inner_iter = i

            self.call_hook("before_estimator_train_iter")

            outputs = self.estimator_batch_processor_inline(
                self.model, self.estimator, data_batch, train_mode=True, **kwargs
            )
            ...
            if "log_vars" in outputs:
                self.log_buffer.update(outputs["log_vars"], outputs["num_samples"])
            self.outputs = outputs
            self.call_hook("after_estimator_train_iter")

            self._iter += 1

        self.call_hook("after_estimator_train_epoch")
        self._epoch += 1
    ...
    def estimator_resume(self, checkpoint, resume_optimizer=True, map_location="default"):
        if map_location == "default":
            checkpoint = self.load_checkpoint(
                checkpoint, map_location=torch.cuda.current_device()
            )
        ...
        while self.epoch < max_epochs:
            for i, flow in enumerate(workflow):
                ...
                for _ in range(epochs):
                    if mode == "train" and self.epoch >= epoch_bias + epochs:
                        continue
                    if mode == "estimator_train" and self.epoch >= epoch_bias + epochs:
                        continue
                    elif mode == "val":
                        epoch_runner(data_loaders[i], **kwargs)
                    else:
                        if mode == "estimator_train":
                            kwargs.update({"epoch_bias": epoch_bias})
    ...
    def register_training_hooks(
        self, lr_config, estimator_lr_config=None, optimizer_config=None, checkpoint_config=None, log_config=None
    ):
        ...
        if estimator_lr_config is not None:
            assert self.estimator_lr_config is None
            self.register_lr_hooks(estimator_lr_config)
        # get the member to realize grad clip manually
