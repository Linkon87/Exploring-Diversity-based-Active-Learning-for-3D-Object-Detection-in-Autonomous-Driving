...
from det3d.torchie.trainer import DistSamplerSeedHook, ActiveTrainer, obj_from_dict
...
def active_train_detector(model, estimator, dataset, cfg, distributed=False, validate=False, logger=None):
    if logger is None:
    ...
    if hasattr(cfg.optimizer, 'TYPE'):
        ...
        estimator_optimizer = build_fastai_optimizer(cfg.estimator_optimizer, estimator)
    else:
        assert hasattr(cfg.optimizer, 'type')
        optimizer = build_optimizer(model, cfg.optimizer)
        estimator_optimizer = build_optimizer(cfg.estimator_optimizer, estimator)
    if hasattr(cfg.lr_config, 'type'):
        lr_scheduler = _create_learning_rate_scheduler(
            optimizer, cfg.lr_config, total_steps
        )
        cfg.lr_config = None
        estimator_lr_scheduler = _create_learning_rate_scheduler(
            estimator_optimizer, cfg.estimator_lr_config, total_steps
        )
        cfg.estimator_lr_config = None
    else:
        lr_scheduler = None
    ...
    if distributed:
        ...
        estimator = DistributedDataParallel(
            estimator.cuda(cfg.local_rank),
            device_ids=[cfg.local_rank],
            output_device=cfg.local_rank,
            # broadcast_buffers=False,
            find_unused_parameters=False,
    ...
    logger.info(f"estimator structure: {estimator}")
    ...
    trainer.register_training_hooks(
        cfg.lr_config, cfg.estimator_lr_config, optimizer_config, cfg.checkpoint_config, cfg.log_config
    )
    ...
    if cfg.resume_from:
        trainer.resume(cfg.resume_from, resume_optimizer=False, map_location="cpu")
    if cfg.estimator_resume_from:
        trainer.estimator_resume(cfg.estimator_resume_from, resume_optimizer=False, map_location="cpu")
    elif cfg.load_from:
        trainer.load_checkpoint(cfg.load_from)
    elif cfg.estimator_load_from:
        trainer.estimator_load_checkpoint(cfg.estimator_load_from)
