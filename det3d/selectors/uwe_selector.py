...
class UWESelector(BaseSelector):
    def __init__(
            self,
            budget: int,
            buffer_file: str,
            ...
            weighted_feat_path: str = '/home/st2000/data/buffers/uwe_feat.pt',
            distance_store_file: str = "/home/st2000/data/buffers/uwe_distance_map.npy",
            p: int = 2,
        ...
        self.weighted_feat_path = weighted_feat_path
    ...
    def buffer_pred(self, **kwargs) -> torch.Tensor:
        self.logger.info(
            f"begin predict all results of samples and save them as {self.weighted_feat_path}")
        ...
        for i, data_batch in enumerate(self.dataloader):
            with torch.no_grad():
                ...
                for b_i, pred in enumerate(preds):
                    ...
                    prog_bar.update()
                    b_id += 1
        entropy_pred_all = torch.stack(entropy_list)

        # entropy_pred_all = torch.load('/home/st2000/data/buffers/entropy_pred2.pt') # make sure calculate entropy first
        uncertainty_norm = (entropy_pred_all - entropy_pred_all.min()) / (entropy_pred_all.max()-entropy_pred_all.min())

        b_id = 0
        prog_bar = torchie.ProgressBar(len(self.dataloader.dataset))
        entropy_weighted_feature_list = []
        for i, data_batch in enumerate(self.dataloader):
            with torch.no_grad():
                if "local_rank" in kwargs:
                    device = torch.device(kwargs["local_rank"])
                ...
                for b_i, (pred, pillar_feat) in enumerate(zip(preds, pillar_feats)):
                    entropy_weighted_feature = pillar_feat * (uncertainty_norm[b_i])#.cpu())
                    entropy_weighted_feature_list.append(entropy_weighted_feature)

                    prog_bar.update()
                    b_id += 1

        prediction = torch.stack(entropy_weighted_feature_list)
        torch.save(prediction, self.weighted_feat_path)
    ...
    def select_samples(self, **kwargs) -> None:
        ...
        if self.pred:
            ...
            self.logger.info(f"all prediction results have been saved in {self.weighted_feat_path}")
        else:
            # load buffer
            feats = torch.load(self.weighted_feat_path)
            # load the prediction results
            self.logger.info(f"all prediction results have been load from {self.weighted_feat_path}")
