...
def create_loss(
    ...
    # iou loss term
    # get the iou of predicted bboxes and gt bboxes
    ...
    batch_task_anchors = anchors[task_id].view(
        batch_size, -1, box_coder.n_dim
    )
    box_preds_decoded = box_coder.decode_torch(
                box_preds[:, :, :box_coder.code_size], batch_task_anchors
            )
    for (anno, box_pred_decode) in zip(annos, box_preds_decoded):
        gt_boxes = np.concatenate(anno["gt_boxes"])
        box_target = torch.from_numpy(np.concatenate([gt_boxes[:, :6], gt_boxes[:, (-1,)]], 1)).to(device)
        box_pred_decode = torch.cat([box_pred_decode[:, :6], box_pred_decode[:, -1:]], dim=-1) # [N, 7]
        ious = boxes_iou3d_gpu(box_pred_decode, box_target) # [N, M]
        iou_target = ious.max(1)[0] # [N]
        iou_targets.append(iou_target[..., None])
    iou_targets = torch.stack(iou_targets, dim=0)
    if isinstance(iou_loss_ftor, WeightedSmoothL1Loss):
        iou_targets = (iou_targets - iou_norm.get("mean")) / iou_norm.get("std")
        iou_losses = iou_loss_ftor(
            iou_preds, iou_targets, weights=reg_weights
        )
    else:
        iou_losses = iou_loss_ftor(
...
class IoUHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.conv_iou = nn.Sequential(
            nn.Conv2d(num_input, 512, 1),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Conv2d(512, num_iou, 1)
        )
        # self.conv_iou = nn.Conv2d(num_input, num_iou, 1)
    ...
    def forward(self, x, finetune=False):
        ...
        iou_preds = self.conv_iou(x).permute(0, 2, 3, 1).contiguous()
        ret_dict = {"box_preds": box_preds, "cls_preds": cls_preds, "iou_preds": iou_preds}
        if self.use_dir:
...
@HEADS.register_module
class MultiGroupIoUHead(nn.Module):
    def __init__(
        self,
        mode="3d",
        in_channels=[128,],
        ...
        loss_iou=dict(type="SmoothL1Loss", beta=1.0, loss_weight=1.0,),
        iou_norm=dict(mean=0.5, std=0.5),
        encode_rad_error_by_sin=True,
        loss_aux=None,
        ...
        for task_id, (num_pred, num_cls, num_iou) in enumerate(zip(num_preds, num_clss, num_ious)):
            self.tasks.append(
                IoUHead(
                    in_channels,
                    num_pred,
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            loc_loss, cls_loss, iou_loss = create_loss(
                self.loss_reg,
                self.loss_cls,
                self.loss_iou,
                self.iou_norm,
            ...
            loss = iou_loss_reduced + loc_loss_reduced + cls_loss_reduced
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if isinstance(self.loss_iou, WeightedSmoothL1Loss):
                batch_iou_preds = batch_iou_preds * self.iou_norm.get("std") + self.iou_norm.get("mean")
