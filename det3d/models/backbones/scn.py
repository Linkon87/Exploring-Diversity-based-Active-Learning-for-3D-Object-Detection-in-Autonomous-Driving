...
#                                  SubMConv3d, SubMConv4d) #高版本spconv，需要debug
...
@BACKBONES.register_module
class FPNSpMiddleResNetFHD(nn.Module):
    def __init__(
        ...
        # input: # [1600, 1200, 41]
        self.middle_conv0 = spconv.SparseSequential(
    ...
    def forward(self, voxel_features, coors, batch_size, input_shape):
        ...
        coors = coors.int()
        ret = spconv.SparseConvTensor(voxel_features, coors, sparse_shape, batch_size)
        ret = self.middle_conv0(ret)
        middle.append(ret)
        ret = self.middle_conv1(ret)
        middle.append(ret)
        ret = self.middle_conv2(ret)
        middle.append(ret)
        ret = self.middle_conv3(ret)
        middle.append(ret)
        ret = ret.dense()
