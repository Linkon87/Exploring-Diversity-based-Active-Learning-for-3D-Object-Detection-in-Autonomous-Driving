...
def main():
    ...
    # to wait the main process samples the dataset
    if args.budget is not None:
        cfg.selector.budget = args.budget
    replace_info_path = cfg.train_anno.replace(".pkl", f"_{cfg.selector.budget}.pkl")
    cfg.data.train.info_path = replace_info_path
    replace_dbinfo_path = cfg.db_sampler.db_info_path.replace(".pkl", f"_{cfg.selector.budget}.pkl")
    cfg.data.train.pipeline[2].cfg.db_sampler.db_info_path = replace_dbinfo_path
    logger.info(f"replace the info path as {cfg.data.train.info_path}")
    logger.info(f"replace the dbinfo path as {cfg.data.train.pipeline[2].cfg.db_sampler.db_info_path}")
