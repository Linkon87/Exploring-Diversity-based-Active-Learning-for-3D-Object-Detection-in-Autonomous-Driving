...
class NuScenes:
    ...
    def __init__(self,
                 ...
                 dataroot: str = '/home/st2000/nuscenes/train',
                 verbose: bool = True,
    ...
    def get_sample_data(self, sample_data_token: str,
        ...
        if selected_anntokens is not None:
            ...
            boxes = self.get_boxes(sample_data_token, list_, show_ann)
    ...
    def get_box(self, sample_annotation_token: str, i:int=0, idx:int=0 ,list_ : list = None,show_ann=False) -> Box:
        """
        ...
        if show_ann:
            ...
            dict_lis = list_
            record = dict_lis[idx][1][i]
            name_old = record['detection_name']
            name = ''
            if name_old == 'car' : name = 'vehicle.car'
            elif name_old == 'truck': name = 'vehicle.truck'
            elif name_old == 'construction_vehicle' : name = 'vehicle.construction'
            elif name_old =='bus' : name = 'vehicle.bus.rigid'
            elif name_old == 'trailer': name = 'vehicle.trailer'
            elif name_old == 'barrier': name = 'movable_object.barrier'
            elif name_old == 'bicycle': name = 'vehicle.bicycle'
            elif name_old == 'motorcycle': name = 'vehicle.motorcycle'
            elif name_old == 'pedestrian': name = 'human.pedestrian.adult'
            elif name_old == 'traffic_cone': name = 'movable_object.trafficcone'
            else: name = 'static.other'

            return Box(record['translation'], record['size'], Quaternion(record['rotation']),
                       name=name ,
                        token=record['sample_token'], score=record['detection_score'])

    def get_boxes(self, sample_data_token: str , list_: list =None, show_ann:bool=False) -> List[Box]:
        """
        ...
        if show_ann:
            ...
            if curr_sample_record['prev'] == "" or sd_record['is_key_frame']:
                ...
                boxes = list(map(lambda token: self.get_box(token, show_ann=show_ann), curr_sample_record['anns']))
                ...
                for curr_ann_rec in curr_ann_recs:
                    ...
                    if curr_ann_rec['instance_token'] in prev_inst_map:
                        ...
                        box = self.get_box(curr_ann_rec['token'],show_ann=show_ann)

                    boxes.append(box)
            return boxes
        else:
            dict_lis = list_
            sam_token = nusc.get('sample_data', sample_data_token)['sample_token']
            idx =  [i for i, t in enumerate(dict_lis) if t[0] == sam_token][0]
            boxes = []
            for i in range(len(dict_lis[idx][1])):
                if dict_lis[idx][1][i]['detection_score'] > 0.15:
                    box = self.get_box(sample_annotation_token='',i=i, idx=idx, list_= dict_lis,show_ann=show_ann)
                    boxes.append(box)
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            _, boxes, _ = self.nusc.get_sample_data(ref_sd_token, box_vis_level=box_vis_level,
                                                    use_flat_vehicle_coordinates=use_flat_vehicle_coordinates,list_=lists,
                                                    show_ann =show_ann)
            ...
            data_path, boxes, camera_intrinsic = self.nusc.get_sample_data(sample_data_token,
                                                                           box_vis_level=box_vis_level,list_=lists,
                                                                           show_ann =show_ann)
...
path2 = '/home/st2000/data/Datasets/nuScenes/train/infos_val_10sweeps_withvelo.pkl'
path = '/home/st2000/data/work_dir/spa_tem_fea_box_plot/prediction.pkl'
path3 = '/home/st2000/data/work_dir/try_plot/infos_val_10sweeps_withvelo.json'
path_entropy= '/home/st2000/data/work_dir/spa_tem_fea_box_plot/entropy/infos_val_10sweeps_withvelo.json'
path_random = '/home/st2000/data/work_dir/spa_tem_fea_box_plot/random/infos_val_10sweeps_withvelo.json'
...
with open(path3,'r',encoding='utf8')as fp:
    spa_tem_fea_val = json.load(fp)
with open(path_entropy,'r',encoding='utf8')as fe:
    entropy_val = json.load(fe)
with open(path_random,'r',encoding='utf8')as fr:
    random_val = json.load(fr)

dict_list = list(spa_tem_fea_val['results'].items())
dict_entropy = list(entropy_val['results'].items())
dict_random = list(random_val['results'].items())


nusc = NuScenes(
    # version="v1.0-trainval", dataroot="/data/Datasets/nuScenes", verbose=True
    version="v1.0-trainval", dataroot="/home/st2000/nuscenes/train", verbose=True
)
#NuScenesExp = NuScenesExplorer(nusc)
#NuScenesExp.map_pointcloud_to_image(pointsensor_token = nusc.get('sample', 'fd8420396768425eabec9bdddf7e64b6')['data']['LIDAR_TOP'],\
#camera_token = nusc.get('sample', 'fd8420396768425eabec9bdddf7e64b6')['data']['CAM_FRONT']
#NuScenesExp.render_pointcloud_in_image(sample_token='fd8420396768425eabec9bdddf7e64b6')
print( " ")

#nusc.render_sample_data('b8fba7d78cf547b996c431dec1f5ee26')
sam_data_token = nusc.get('sample', 'fd8420396768425eabec9bdddf7e64b6')['data']['CAM_FRONT']
nusc.render_sample_data(sample_data_token=sam_data_token, lists=dict_entropy,show_ann=True)
## LIDAR_TOP CAM_FRONT CAM_FRONT_RIGHT CAM_FRONT_RIGHT CAM_BACK_LEFT CAM_BACK_RIGHT CAM_BACK
