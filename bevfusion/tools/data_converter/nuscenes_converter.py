...
def create_nuscenes_infos(
    root_path, info_prefix, budget,buffer_path,version="v1.0-trainval", max_sweeps=10
):
    ...
    # )
    train_nusc_infos = mmcv.load('/dataset/1nuscenes_infos_train.pkl')['infos']

    metadata = dict(version=version)

    # select data
    # needs: in parser.add_argument
    budget = budget  #1200
    buffers_path =  buffer_path  #'/home/st2000/data/buffers/spatial_temporal_feature.json'
    print("budget:",budget)
    print("buffers_path:", buffers_path)
    with open(buffers_path, 'r', encoding='utf8') as fe:
        buffer = json.load(fe)
    train_nusc_infos = [train_nusc_infos[i] for i in buffer[str(budget)] ]
